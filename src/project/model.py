...
class HubertClassifier(nn.Module):
    ...
    def __init__(
        self, model_name: str = "ntu-spml/distilhubert", num_labels: int = 8, freeze_feature_encoder: bool = True
    ):
        super().__init__()
    ...
    def forward(
        ...
        outputs = self.hf_model(
            input_values=input_values, attention_mask=attention_mask, labels=labels, return_dict=True
        )
    ...
    def predict(self, audio_input: Union[str, Path, np.ndarray, torch.Tensor]) -> dict:
        ...
        inputs = self.feature_extractor(
            waveform, sampling_rate=self.target_sampling_rate, return_tensors="pt", padding=True
        )
